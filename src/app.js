...
crew = data => {
  const crew = Object.values(data.crew);
  crew.forEach(cr => {
...
technology = data => {
  const technology = Object.values(data.technology);
      ...
      button.addEventListener('click', e => {
        ...
        if (e.target.id == tech.name.split(' ', 1)) {
          ...
          if (windowSize.matches) {
            imgRocket.style.backgroundImage = `url('../${tech.images.landscape}')`;
          } else {
            imgRocket.style.backgroundImage = `url('../${tech.images.portrait}')`;
          }
